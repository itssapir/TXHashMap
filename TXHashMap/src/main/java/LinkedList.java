...
public class LinkedList {
	...
	protected void printLinkedListNotInTX() {
		...
		while (n != null) {
			...
			if (n.isDeleted()) {
				System.out.println("***node " + n + " " + n.key + " is deleted***");
			}
			if (n.isLocked()) {
				System.out.println("***node " + n.key + " is locked***");
			}		
			n = n.next;
		}
		System.out.print("\n");
	...
	private Object getVal(LNode n, LocalStorage localStorage) {
		if (TX.CLOSED_NESTING && localStorage.TXnum > 1) {
			// read from inner write set
			WriteElement we = localStorage.innerWriteSet.get(n);
			if (we != null) {
				return we.val;
			}
		}
	...
	private LNode getPred(LNode n, LocalStorage localStorage) throws TXLibExceptions.AbortException {
		...
		while (true) {
			if (pred.isLocked() || pred.getVersion() > readVersion) {
				// abort TX
				if (TX.CLOSED_NESTING && localStorage.TXnum > 1) {
			...
			if (pred.isSameVersionAndSingleton(readVersion)) {
			...
			if (TX.CLOSED_NESTING && localStorage.TXnum > 1) {
				// just need to find a pred that was not deleted
				WriteElement we = localStorage.innerWriteSet.get(pred);
				if (we != null) {
					if (we.deleted) {
	...
	private LNode getNext(LNode n, LocalStorage localStorage) throws TXLibExceptions.AbortException {
		...
		// try to read from private write set
		WriteElement we = localStorage.writeSet.get(n);
		if (we != null) {
			return we.next;
		}
		...
		LNode next = n.next;
		unsafe.loadFence();
		if (n.isLocked() || n.getVersion() > readVersion) {
			// abort TX
			if (TX.CLOSED_NESTING && localStorage.TXnum > 1) {
				localStorage.innerTX = false;
			} else {
		...
		if (n.isSameVersionAndSingleton(readVersion)) {
			TX.incrementAndGetVersion();
			if (TX.CLOSED_NESTING && localStorage.TXnum > 1) {
				localStorage.innerTX = false;
			} else {
	...
	public Object put(Integer key, Object val) throws TXLibExceptions.AbortException {
		...
		if (found) {
			if (TX.CLOSED_NESTING && localStorage.TXnum > 1) {
				WriteElement we = localStorage.innerWriteSet.get(next);
				if (we != null) {
					// if it is already in write set then just change val
					localStorage.putIntoInnerWriteSet(next, we.next, val, we.deleted);
				} else {
					localStorage.putIntoInnerWriteSet(next, next.next, val, false);
				}
				localStorage.innerReadSet.add(next);
				return next.val;
			}
		...
		if (TX.CLOSED_NESTING && localStorage.TXnum > 1) {
			localStorage.putIntoInnerWriteSet(pred, n, getVal(pred, localStorage), false);
			localStorage.addToInnerIndexAdd(this, n);

			// add to read set
			localStorage.innerReadSet.add(pred);
			return null;
		}
	...
	public Object putIfAbsent(Integer key, Object val) throws TXLibExceptions.AbortException {
		...
		if (TX.CLOSED_NESTING && localStorage.TXnum > 1) {
			localStorage.putIntoInnerWriteSet(pred, n, getVal(pred, localStorage), false);
			localStorage.addToInnerIndexAdd(this, n);
			localStorage.innerReadSet.add(pred); // add to read set
			return null;
		}
	...
	private Object removeSingleton(Integer key) {
		...
		while (true) {
			...
			next = pred.next;
			unsafe.loadFence();
			if (pred.isLockedOrDeleted()) continue;
	...
	public Object remove(Integer key) throws TXLibExceptions.AbortException {
		...
		if (found) {
			...
			if (TX.CLOSED_NESTING && localStorage.TXnum > 1) {
				localStorage.putIntoInnerWriteSet(pred, getNext(next, localStorage), getVal(pred, localStorage), false);
				localStorage.putIntoInnerWriteSet(next, null, getVal(next, localStorage), true);
				// add to read set
				localStorage.innerReadSet.add(next);
				localStorage.addToInnerIndexRemove(this, next);
